...
function Start-Node {
    ...
    $arguments = @('--port', $Port, '--dir', $dataDir, '--nodeid', $Name, '--save', '') + $ExtraArgs
...
function Invoke-Cmd {
    ...
    return (& $cli -p $Port @Arguments 2>&1 | Out-String).Trim()
}
...
try {
    ...
    Assert-Match "TTL is reported before expiry" (Invoke-Cmd $port @('PTTL', 'temp')) '\d'
    ...
    $multiScript = "MULTI`nSET tx 1`nINCR tx`nEXEC`nGET tx`nexit`n"
    $multiOutput = ($multiScript | & $cli -p $port 2>&1 | Out-String)
    Assert-Match "queued commands run on EXEC" $multiOutput '"2"'

    $abortScript = "MULTI`nNOSUCHCOMMAND x`nEXEC`nexit`n"
    $abortOutput = ($abortScript | & $cli -p $port 2>&1 | Out-String)
    ...
    Assert-Match "a TTL survives a crash" (Invoke-Cmd $port @('TTL', 'expiring')) '\d\d\d'
    ...
    $filler = 'x' * 1024
    for ($i = 0; $i -lt 4000; $i++) {
        & $cli -p $port SET "fill:$i" $filler | Out-Null
    }
    ...
    $sawOom = $false
    for ($i = 0; $i -lt 4000; $i++) {
        $result = & $cli -p $port SET "fill:$i" $filler 2>&1 | Out-String
        if ($result -match 'OOM') { $sawOom = $true; break }
    }
    Assert-Match "noeviction refuses writes with OOM" ($(if ($sawOom) { 'OOM' } else { 'no OOM seen' })) 'OOM'
    Assert-Match "reads still work when out of memory" (Invoke-Cmd $port @('DBSIZE')) '\d'
